...
# ╔═╡ 07e84a10-08a3-11ef-2ee5-a3f06440d61e
using Turing, FillArrays, LinearAlgebra, CSV, DataFrames, MLDataUtils

# ╔═╡ 15795f45-743b-423f-81e4-1c23054a1f23
using Plots, StatsPlots

# ╔═╡ 9a5e5e93-6447-4bcf-a350-5cd1d5b29c1c
using MultivariateStats 

# ╔═╡ b700fd41-d4d1-49b1-a7d5-b9c91b2ed6f1
include("getPrincipalComponents.jl")

# ╔═╡ afaaa4d7-9dd8-4036-bb89-64ac68a9448d
data = CSV.read("data_after_pca.csv", DataFrame)

# ╔═╡ e07ee895-d4ed-41d2-aa01-4ef35c4c7eee
train, test = splitobs(shuffleobs(data), at = 0.9)

# ╔═╡ 85990497-0c6b-4d35-b24a-44ecb0451b92
...
# ╔═╡ 5991475e-764d-4d20-8e7c-7d636d474d92
# Bayesian linear regression.
@model function heteroskedastic_regression(x, y)
	# Set std prior.
    σ_int ~ Exponential(1)
	σ_slope ~ Normal(0,1)

	nRows, nFeatures = size(x)
    # Set the priors on our coefficients.
	coefficients = Matrix(undef,1,nFeatures)
    for i in 1:length(coefficients)
		coefficients[i] ~ Exponential(2)
	end
	
    for i in 1:nRows
		mu = (coefficients * x[i,:])[1]
		σ = σ_int * exp(σ_slope * mu)
        y[i] ~ Normal(mu, σ)
    end
end

# ╔═╡ 125bec34-8180-419a-adad-1e1c0a35e4e7
begin
	scatter(
		data[:,[:Pattern_Recognition]][:,1],
		data[:,[:Calculation]][:,1],
		markersize=2,
		markeralpha=.3,
		markerstrokewidth=.5,
	...
	xaxis!("predictor 1")
	yaxis!("predictor 2")
	title!("Relationship of predictors")
end

# ╔═╡ 7b6b9acc-114e-49b9-ba8b-055962b84097
normalize = x -> (x .- mean(x)) ./ std(x)

# ╔═╡ c68a6a2f-7535-4b1a-91cf-9dacee7081e0
denormalize = prev -> x -> (x .* std(prev)) .+ mean(prev)

# ╔═╡ 1e837bde-b9ab-436d-a642-572943f59406
function generateScatterPlots(X,y, column)
	r1 = round(cor(X[:,1],y)[1]^2,digits=3)
	r2 = round(cor(X[:,2],y)[1]^2,digits=3)
	plot1 = scatter(
		X[:,1],
		y,
		markersize=2,
	...
	xaxis!(plot1, "Pattern Recognition")
	yaxis!(plot1, "$column Rating")
	title!(plot1, "Pattern Recognition vs $column")
	...
	xaxis!(plot2, "Calculation")
	yaxis!(plot2, "$column Rating")
	title!(plot2, "Calculation vs $column")
	return plot1, plot2
end

# ╔═╡ 6f013879-ebff-46bb-a754-8aaa912dc1b0
function generateModel1(df, column, minDeviation)
	colToFilter = "$column" * "_deviation"
	filtered_data = filter(
		row -> row["$colToFilter"] < minDeviation, df
	)
	
	X = Matrix(filtered_data[:,[:Pattern_Recognition, :Calculation]])
	y = hcat(normalize(filtered_data[:,["$column"]][:,1]))
	
	denorm = denormalize(filtered_data[:,["$column"]][:,1])
	...
	mean_β = [mean(chain[:"coefficients[1]"]),mean(chain[:"coefficients[2]"])]
	mean_yhat = X * mean_β

	pre_regression_plots = generateScatterPlots(X,y, column)
	...
	xaxis!(plot1, "Actual Rating")
	yaxis!(plot1, "Predicted Rating")
	title!(plot1, uppercase(column))

	plot2 = density(1 .- chain[:"σ²"], legend=nothing)
	title!(plot2, "% of $column variance explained")

	rsquared = var(mean_yhat) / var(y)
	# http://www.stat.columbia.edu/~gelman/research/unpublished/bayes_R2_v3.pdf
	alt_rsquared = var(mean_yhat) / (var(mean_yhat) + var(y .- mean_yhat))

	plot3 = density(chain[:"coefficients[1]"],label="Pattern Recognition")
	density!(chain[:"coefficients[2]"],label="Calculation")
	title!("stddev change in $column / stddev change in predictor")
	...
	xaxis!(plot4, "Fitted")
	yaxis!(plot4, "Residual")
	title!(plot4, uppercase(column))

	function posteriorPredict(input)
		result = []
		for i in 1:length(chain)
			β = [chain[i][:"coefficients[1]"][1],chain[i][:"coefficients[2]"][1]]
			mean = input * β
			push!(
				result,
				denorm(rand(Normal(
					mean[1],sqrt(chain[i][:"σ²"][1])
				)))
			)
		end
		return (
			quantile(result,0.025),
			quantile(result,0.5),
			quantile(result,0.975)
		)
	end

	return chain, (pre_regression_plots..., plot1, plot2, plot3, plot4), denorm, posteriorPredict, rsquared, alt_rsquared
end

# ╔═╡ 910f1f0a-297c-4403-8de3-0336cc91f47b
function generateModel2(df, column, minDeviation)
	colToFilter = "$column" * "_deviation"
	filtered_data = filter(
		row -> row["$colToFilter"] < minDeviation, df
	)
	...
	chain = sample(
		heteroskedastic_regression(X,y),
		NUTS(),
		3_000
	)
	...
	plot2 = density(
		1 .- (chain[:σ_int] .* exp.(-2 * chain[:σ_slope])).^2,
		label="2 std dev below mean"
	)
	density!(
		1 .- (chain[:σ_int] .* exp.(0 * chain[:σ_slope])).^2,
		label="mean"
	)
	density!(
		1 .- (chain[:σ_int] .* exp.(2 * chain[:σ_slope])).^2,
		label="2 std dev above mean"
	)
	title!("% of variance explained, by distance from mean rating")
	...
	plot!(
		-5:5,
		x -> 2 * (mean(chain[1][:σ_int]) * exp(x * mean(chain[1][:σ_slope]))),
		label="2 std below"
	)
	plot!(
		-5:5,
	...
	function posteriorPredict(input)
		...
		for i in 1:length(chain)
			β = [chain[i][:"coefficients[1]"][1],chain[i][:"coefficients[2]"][1]]
			mean = input * β
			σ = chain[i][:"σ_int"][1] * exp(chain[i][:"σ_slope"][1] * mean[1])
			push!(
				result,
				denorm(rand(Normal(
					mean[1],σ
...
# ╔═╡ 41b2aad5-3e3c-47e2-9e6c-745d2b031308
function plotPosteriorPrediction(input, chain, denorm)
	isHeteroskedasticModel = :σ_slope in names(chain)
	result = []
	for i in 1:length(chain)
		β = [chain[i][:"coefficients[1]"][1],chain[i][:"coefficients[2]"][1]]
		mean = input * β
		σ = isHeteroskedasticModel ? 
			chain[i][:"σ_int"][1] * exp(chain[i][:"σ_slope"][1] * mean[1]) : sqrt(chain[i][:"σ²"][1])
		push!(
			result,
			denorm(rand(Normal(
				mean[1],σ
	...
	post = density(result,label=nothing)
	vline!(post,[
		quantile(result,0.025),
		quantile(result,0.975)
	],label="95% Interval")
...
# ╔═╡ 30879134-bbfb-4c97-ab36-677d1aa9df1c
function validateModel(model, column, test_data, minDeviation)
	colToFilter = "$column" * "_deviation"
	filtered_data = filter(
		row -> row["$colToFilter"] < minDeviation, test_data
	)

	yhat = model[4].(
		[reshape(row, 1, 2) for row in eachrow(
			Matrix(filtered_data[:,[:Pattern_Recognition,:Calculation]])
		)]
	)
	...
	for i in 1:length(yhat)
		interval = yhat[i]
		truth = Matrix(filtered_data[:,["$column"]])[i,:][1]
		
		if truth > interval[1] && truth < interval[3]
			correct += 1
		end

		if interval[2] < mean(Matrix(filtered_data[:,["$column"]]))
			push!(
				belowMean,
				(truth > interval[1] && truth < interval[3]) ? 1 : 0
			)
		else
			push!(
				aboveMean,
	...
	accuracy = correct / length(yhat)
	belowVsAboveMeanAccuracy = (
		sum(belowMean)/length(belowMean),
		sum(aboveMean)/length(aboveMean)
	)

	errorDensity = density(
		map(x -> x[2], yhat) .- Matrix(filtered_data[:,["$column"]]),
		label=nothing,
	)
	title!(errorDensity, "Distribution of residuals")

	numIntervals = min(100,length(yhat))
	intervals = [(p[2]-p[1],p[3]-p[2]) for p in yhat[1:numIntervals]]
	intervalPlot = scatter(
		1:numIntervals,
		map(x -> x[2], yhat[1:numIntervals]),
		yerror = intervals,
		markersize = 1.75,
		markeralpha = .5,
		markerstrokewidth = .5,
	...
	scatter!(
		1:numIntervals,
		filtered_data[:,["$column"]][:,1],
		markersize = 2.25,
		markerstrokewidth = .5,
		color = "red",
		label="Actual"
	)
	title!(intervalPlot, "Prediction Intervals on Test Split")
	return accuracy, belowVsAboveMeanAccuracy, errorDensity, intervalPlot
end

# ╔═╡ c00ae49b-ac4a-4eea-9d7e-ea6600f09c60
md"""
# Classical
"""

# ╔═╡ e961c6ba-d9b9-4185-9fca-297550bd44c4
classicalModel = generateModel1(train, "classical", 80)

# ╔═╡ 6b9a99b6-25f0-4902-a2a6-7021a42173b8
classicalModel[2]

# ╔═╡ 5e3bc556-381d-42d1-9db2-a684597bceb9
classicalModel[2][5]

# ╔═╡ 0444eec5-be93-4686-b0ab-287ba9f3f9ac
classicalValidation = validateModel(classicalModel, "classical", test, 110)

# ╔═╡ b292aa42-c360-4306-8774-0dac7ee11b23
classicalValidation[4]

# ╔═╡ a0b2d1e6-1ed5-4066-bbe9-3abd9a29a863
md"""
# Rapid
"""

# ╔═╡ 45d7e48d-c67e-4bb7-aa76-fe91af93d2c1
rapidModel1 = generateModel1(train, "rapid", 80)

# ╔═╡ 44686916-9e49-42d4-bead-b3ed4c87f941
rapidModel1[2]

# ╔═╡ bed038e2-6622-4d7b-8871-781239397c9c
rapidModel1[2][5]

# ╔═╡ f2467322-a559-43b0-b323-22dc03b1e15f
rapidValidation1 = validateModel(rapidModel1, "rapid", test, 80)

# ╔═╡ a7c6d474-8478-4b90-817e-21686767dff1
rapidValidation1[4]

# ╔═╡ d5f97c0a-eaad-43c1-bd87-c4637b638657
rapidModel2 = generateModel2(train, "rapid", 80)

# ╔═╡ eeeafdf0-2437-4dc6-88f3-1210e75adaaf
rapidModel2[2]

# ╔═╡ feb19060-54b0-4084-961c-5bf7c54036f8
rapidModel2[2][5]

# ╔═╡ 4abbbb23-3660-4bc3-bc00-232e5cafa887
rapid2Validation = validateModel(rapidModel2,"rapid",test,60)

# ╔═╡ fd93cd8f-cc21-41bb-ba69-0527dc0c755c
rapid2Validation[4]

# ╔═╡ 0f2e397e-b77b-46b5-b813-6f515f825013
md"""
###### posterior prediction interval for 2 std dev **above** mean in both predictors
"""

# ╔═╡ 56546a07-747c-48d7-ba22-c1bf72a46432
md"""
__Model 1__
"""

# ╔═╡ 60fe356e-4c8f-471b-9625-6803f8bb9e70
plotPosteriorPrediction(
	hcat(2,2),
	rapidModel1[1],
	rapidModel1[3]
)

# ╔═╡ e8e7df28-19c5-4f24-bb66-470854363b26
md"""
__Model 2__
"""

# ╔═╡ 3335154a-489d-4191-9a96-484f60a4b810
plotPosteriorPrediction(
	hcat(2,2),
	rapidModel2[1],
	rapidModel2[3]
...
# ╔═╡ fe56c685-e5fe-4c2e-84a8-016c9870c49d
md"""
Means are the same but model 2 has narrower intervals because its above the mean
"""

# ╔═╡ 32be370c-cc42-4608-a3ad-ba0245d233d4
md"""
# Blitz
"""

# ╔═╡ 3dce4f89-e863-48e1-87dd-0da7c5dc0b75
blitzModel1 = generateModel1(train, "blitz", 80)

# ╔═╡ 69f6f43d-943b-4b7c-9db3-cfdd9fa800c6
blitzModel1[2][5]

# ╔═╡ 45b638bd-1b29-4f7a-ade4-234c57eb2058
validateModel(blitzModel1, "blitz", test, 70)

# ╔═╡ 3435c31e-52c0-4968-8227-fc17f035c7cb
blitzModel2 = generateModel2(train, "blitz", 80)

# ╔═╡ 1cd351df-5ca6-4321-b01c-946968771deb
validateModel(blitzModel2, "blitz", test, 70)

# ╔═╡ d8a713c2-5861-4cbe-9f22-3416fad0759e
md"""
# Bullet
"""

# ╔═╡ 8c553bcd-43ae-436e-8126-6f808197624f
bulletModel1 = generateModel1(train, "bullet",70)

# ╔═╡ d1e2b68b-b08d-4004-b879-140dc723b3a4
bulletModel1[2][5]

# ╔═╡ 2c3a9b32-a4b0-40ef-ac50-91dc192b6eee
validateModel(bulletModel1, "bullet", test, 70)

# ╔═╡ 62a09000-e4fa-4739-9035-139ef5f0972c
md"""
##### Including PCA file to predict ratings from your scores
"""

# ╔═╡ c37a3a06-130e-4113-8e28-56038208d538
pcaResults = results

# ╔═╡ 327cb394-4499-4a07-ad2d-d75210b8cdb8
function predict(model, storm, racer, streak, puzzle)
	input = reduceScoreToComponents(storm, racer, streak, puzzle)
	interval = model[4](input)
	plot = plotPosteriorPrediction(
		input,
		model[1],
		model[3]
	)
...
# ╔═╡ 574b2cb6-f71b-4523-9f92-f93e54d31187
myPrediction = predict(rapidModel1, 27, 33, 30, 2000)

# ╔═╡ 537ee2c7-fcbe-46e5-992a-924c4a3ae5ca
myPrediction[2]
...
julia_version = "1.9.4"
manifest_format = "2.0"
project_hash = "d600d90808c34d7f8db7aa6930c57783a8cea641"
